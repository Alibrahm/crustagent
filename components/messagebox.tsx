...
const MessageBox = ({ role, content }: Props) => {
  return (
    <Card className="overflow-hidden">
      {role === "user" ? (
        <CardContent className="flex flex-row justify-end border-none bg-muted/50 px-6  w-auto">
          <Markdown text={content} role={role} />
        </CardContent>
      ) : (
      ...
      {role !== "user" && (
        <CardFooter className="border-t bg-muted/50 px-6 py-3 text-xs text-muted-foreground">
          {/* Disclaimer: This is only from the data fed from th KB, the LLM can infer*/}
        </CardFooter>
      )}
    </Card>
  );
